<script lang="ts">
	import type { Subtask, SubtaskCreate, SubtaskUpdate, Todo, TodoUpdate } from '$lib/types';
	...
	interface Props {
		...
		onToggleSubtask: (todoId: number, subtaskId: number, completed: boolean) => void | Promise<void>;
		onCreateSubtask: (todoId: number, subtask: SubtaskCreate) => void | Promise<void>;
		onDeleteSubtask: (todoId: number, subtaskId: number) => void | Promise<void>;
	}

	let { todo, onToggle, onSave, onDelete, onToggleSubtask, onCreateSubtask, onDeleteSubtask }: Props = $props();
	...
	async function addSubtask(e: Event) {
		e.preventDefault();
		const title = newSubtaskTitle.trim();
		if (!title) return;
		const position = (todo.subtasks?.length || 0);
		await onCreateSubtask(todo.id, { title, position });
		newSubtaskTitle = '';
	}
</script>
		...
		{:else}
			...
			>
				...
				<div class="min-w-0 flex-1">
							...
							{#each todo.subtasks as subtask (subtask.id)}
								<div class="flex items-center gap-2">
									<input
										...
										onchange={(e) => onToggleSubtask(todo.id, subtask.id, e.currentTarget.checked)}
										class="h-4 w-4 shrink-0 cursor-pointer rounded border-slate-300 text-indigo-600 focus:ring-2 focus:ring-indigo-500"
									/>
									<span
										class="text-sm text-slate-700 break-words"
										class:line-through={subtask.completed}
										class:text-slate-400={subtask.completed}
									>
										{subtask.title}
									</span>
									<button
					...
					{#if expanded}
						<form onsubmit={addSubtask} class="mt-2 ml-4">
							<div class="flex gap-2">
								<input
									type="text"
